...
class MGSTGNN(nn.Module):
    def __init__(
        ...
        self.adj,self.dis = torch.Tensor(adj),torch.Tensor(dis)
    ...
    def forward(self, source):
        ...
        _, output = self.predictor(source[...,:self.num_input_dim], init_state, self.adj) # B, T, N, hidden
...
class DSTRNN(nn.Module):
    ...
    def forward(self, x, init_state, metrix):
        # shape of x: (B, T, N, D)
        ...
        for i in range(len(self.num_grus)):
            ...
            for t in range(seq_length):
                ...
                init_hidden_states[index1+index2] = self.grus[index1+index2](inp, prev_state, metrix) # [B, N, hidden_dim]
...
class GCN(nn.Module):
    def __init__(self, dim_in, dim_out, embed_dim, node_num):
        ...
        self.node_num = node_num
        self.weights = nn.Parameter(torch.FloatTensor(dim_in, dim_out)) # [D, C, F]
        self.bias = nn.Parameter(torch.FloatTensor(node_num, dim_out)) # [D, F]
        self.norm = nn.LayerNorm(embed_dim, eps=1e-12)
        self.drop = nn.Dropout(0.1)
    def forward(self, x, metrix):
        metrix = metrix.to(x.device)
        ...
        supports2 = GCN.get_laplacian(metrix,supports1)
        x_g1 = torch.einsum("nm,bmc->bnc", supports1, x)
        x_g2 = torch.einsum("nm,bmc->bnc", supports2, x)
        x_g = x_g1 + x_g2   #b,n,d
        x_gconv = torch.einsum('bni,io->bno', x_g, self.weights) + self.bias.unsqueeze(0)  #b, N, dim_out
...
class Network(nn.Module):
    def __init__(self, args):
        ...
        self.mgstgnn = MGSTGNN(args.num_nodes,args.batch_size,args.input_dim,args.rnn_units,args.output_dim,args.num_grus,args.embed_dim,
            ...
            weekend_embedding_dim=args.weekend_embedding_dim,num_input_dim=args.num_input_dim,adj=args.adj,dis=args.dis)
    def forward(self,x):
...
if __name__ == "__main__":
    ...
    args.adj = np.ones((args.num_nodes,args.num_nodes))
    args.dis = np.ones((args.num_nodes,args.num_nodes))
