...
class Encoder(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.periods_embedding_dim > 0:
            ...
            time_embedding = torch.mul(time_embedding, periods_emb[0,:,0])
        if self.weekend_embedding_dim > 0:
            weekend_emb = self.weekend_embedding(
                weekend.long()
            )  # (batch_size, in_steps, num_nodes, weekend_embedding_dim)
            time_embedding = torch.mul(time_embedding, weekend_emb[0,:,0])
...
class GCN(nn.Module):
    ...
    def forward(self, x, embeddings):
        ...
        supports2 = F.softmax(torch.matmul(embedding, embedding.transpose(0, 1)), dim=1)    # b,n,n
        x_g1 = torch.einsum("nm,bmc->bnc", supports1, x)
        x_g2 = torch.einsum("nm,bmc->bnc", supports2, x)
        x_g = torch.stack([x_g1,x_g2],dim=1)        # b,2,n,d
        weights = torch.einsum('nd,dkio->nkio', embedding, self.weights_pool) # N, dim_in, dim_out
        bias = torch.einsum('nd,do->no', embedding, self.bias_pool) # b, N, dim_out
        x_g = x_g.permute(0, 2, 1, 3)  # B, N, cheb_k, dim_in
        x_gconv = torch.einsum('bnki,nkio->bno', x_g, weights) + bias.unsqueeze(0)  #b, N, dim_out
